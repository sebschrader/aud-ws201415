...
inline int parser_peek(parser_t *p, char *c) {
    if (p->pos == p->len) return PARSER_EOF;
    *c = p->str[p->pos];
    return PARSER_OK;
}

inline void parser_eat_space(parser_t *p) {
    while(isspace(p->str[p->pos])) p->pos++;
}

inline int parser_eat_char(parser_t *p) {
    if (p->pos == p->len) return PARSER_EOF;
    p->pos++;
    return PARSER_OK;
}

inline int parser_expect_char(parser_t *p, char expected) {
    char got;
    int rc;
    if ((rc = parser_peek(p, &got)) != PARSER_OK) return rc;
    if (expected != got) {
...
inline int parser_read_int(parser_t *p, int *v) {
    long n;
...
tree parse_tree(const char *line) {
    parser_t p = { line, strlen(line), 0 };
    tree t = NULL;
    parser_read_tree(&p, &t);
    return t;
}
